...
namespace Core.UI
{
    public partial class FormManualScreen : Form
    {
        ...
        // BlueTool 검사
        private void btn_RunBlueTool_Click(object sender, EventArgs e)
        {
            cogRecordDisplay = CMainFrame.mCore.m_FuncInspection.RunBlueTool(cogAcquireImage);
            rd_MainDisplay_Cam0.Record = cogRecordDisplay;
            rd_MainDisplay_Cam0.Fit();
        }

        private void btn_RunGreenTool_Click(object sender, EventArgs e)
        {
            CMainFrame.mCore.m_FuncInspection.RunGreenTool(cogAcquireImage, out CogRecord cogRecordDisplay, out string sbestTag);

            rd_MainDisplay_Cam0.Record = cogRecordDisplay;
            rd_MainDisplay_Cam0.Fit();

            txt_BestTag.Text = sbestTag;
        }
    }
}